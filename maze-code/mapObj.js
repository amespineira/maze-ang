...
function MapObj(specs){
  ...
  this.map=copyObjectValue(this.init);
...
MapObj.prototype.solveMap= function(){
  ...
  var outputMap=copyObjectValue(this.init);
...
MapObj.prototype.generateRelativeMap = function () {
  var condition=true;
  var relativeMap=copyObjectValue(this.init);
  var foundx=false;
...
function copyObjectValue(input){
  if(Array.isArray(input)){
    var output=[];
    input.forEach(function(value){
      output.push(copyObjectValue(value))
    })
    return output;
  }
    ...
    for(var key in input){
      output[key]=copyObjectValue(input[key]);
    }
    return output;
  }
  else{
